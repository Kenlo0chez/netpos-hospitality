...
Write-Host ""
Write-Host "NETPOS HOSPITALITY - FUNCTIONALITY & SECURITY BATCH" -ForegroundColor Cyan
Write-Host "Billing Accounts + Refund Permissions + Secure Staff Creation" -ForegroundColor Cyan
Write-Host ""

$root = (Get-Location).Path

function Write-FullFile([string]$RelativePath, [string]$Content) {
    ...
    if ([System.IO.File]::Exists($fullPath)) {
        $backup = $fullPath + ".security-batch-backup"
        if (-not [System.IO.File]::Exists($backup)) {
            [System.IO.File]::Copy($fullPath, $backup, $false)
            Write-Host "Backup: $RelativePath.security-batch-backup" -ForegroundColor DarkGray
        }
    }
...
Write-FullFile "app\billing\page.tsx" $billing
Write-FullFile "app\billing\accounts\page.tsx" $accounts
Write-FullFile "app\reservations\[id]\page.tsx" $folio
Write-FullFile "app\api\staff\create\route.ts" $staffRoute

Write-Host ""
Write-Host "FUNCTIONALITY & SECURITY BATCH COMPLETE" -ForegroundColor Green
Write-Host ""
Write-Host "IMPORTANT: the staff creation route now requires the Users page to send the signed-in access token." -ForegroundColor Yellow
Write-Host "We will verify that next before changing any database policies." -ForegroundColor Yellow
Write-Host ""
