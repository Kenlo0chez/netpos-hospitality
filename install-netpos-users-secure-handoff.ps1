...
Write-Host ""
Write-Host "NETPOS HOSPITALITY - USERS SECURE API HANDOFF" -ForegroundColor Cyan
Write-Host ""

$root = (Get-Location).Path
$relative = "app\users\page.tsx"
$path = Join-Path $root $relative

if (-not [System.IO.File]::Exists($path)) {
    throw "Missing $relative. Run this from C:\Users\Administrator\netpos-hospitality"
}

$backup = $path + ".secure-handoff-backup"
if (-not [System.IO.File]::Exists($backup)) {
    [System.IO.File]::Copy($path, $backup, $false)
    Write-Host "Backup: $relative.secure-handoff-backup" -ForegroundColor DarkGray
}
...
Write-Host "Updated: app\users\page.tsx" -ForegroundColor Green
Write-Host ""
Write-Host "USERS SECURE API HANDOFF COMPLETE" -ForegroundColor Green
Write-Host ""
