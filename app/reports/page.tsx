...
export default function ReportsPage() {
  ...
  useEffect(() => {
    setOwnerWhatsApp(
      localStorage.getItem("netpos_owner_whatsapp") ?? ""
    );
    initialise();
  }, []);
  ...
  const arrivals = stayReservations.filter(
    (reservation) =>
      reservation.arrival_date >= startDate &&
      reservation.arrival_date <= endDate
  ).length;

  const departures = stayReservations.filter(
    (reservation) =>
      reservation.departure_date >= startDate &&
  ...
  function sendWhatsAppReport() {
    ...
    if (phone.length < 8) {
      setErrorMessage(
        "Enter the owner's WhatsApp number with country code, for example 264811234567."
      );
      return;
    }
    ...
    const report = [
      ...
      "",
      "*PERFORMANCE*",
      `Accommodation revenue: ${money(accommodationRevenue)}`,
      `Occupancy: ${occupancyAverage.toFixed(1)}%`,
      `Room nights: ${roomNightsSold} sold / ${roomNightsAvailable} available`,
      `Active stays: ${stayReservations.length}`,
      `Guests: ${totalGuests} (${totalAdults} adults, ${totalChildren} children)`,
      `Arrivals: ${arrivals}`,
      ...
      `EFT: ${money(eftTotal)}`,
      `Account: ${money(accountTotal)}`,
      `Gross payments: ${money(grossPayments)}`,
      `Refunds: ${money(refundTotal)}`,
      `Net payments: ${money(netPayments)}`,
      "",
      "*CONTROL*",
      `Outstanding: ${money(outstanding)} (${outstandingReservations} reservation${outstandingReservations === 1 ? "" : "s"})`,
      `Cancellations: ${cancelledReservations.length}`,
      `No shows: ${noShowReservations.length}`,
      "",
      "_Generated by Netpos Hospitality_",
    ].join("\n");
          ...
          <input
            ...
            onBlur={() =>
              localStorage.setItem(
                "netpos_owner_whatsapp",
                ownerWhatsApp.replace(/\D/g, "")
              )
            }
            placeholder="Owner WhatsApp: 264..."
