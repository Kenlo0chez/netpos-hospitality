...
export default function FinancePage() {
  ...
  return <main style={page}><section style={shell}>
    <header style={header}><div><div style={eyebrow}>FINANCE CONTROL</div><h1 style={title}>Cashbook & Reconciliation</h1>
      ...
      <select value={propertyId} onChange={(e) => changeProperty(e.target.value)} style={propertySelect} aria-label="Property">
  ...
  return <section style={{ ...panel, marginTop: 12 }}><div style={panelHeading}><div><h2 style={panelTitle}>{title}</h2><p style={panelText}>{description}</p></div><span style={countBadge}>{entries.length} entries</span></div>
    <div style={historyWrap}><table style={table}><thead><tr><th style={th}>Date</th><th style={th}>Type</th><th style={th}>Description</th><th style={th}>Method</th><th style={th}>Reference</th><th style={th}>VAT</th><th style={amountTh}>Amount</th>{reconciliation && <th style={th}>Bank status</th>}</tr></thead>
      <tbody>{loading ? <tr><td colSpan={8} style={emptyCell}>Loading finance entries...</td></tr> : entries.length === 0 ? <tr><td colSpan={8} style={emptyCell}>No processed entries for this property.</td></tr> : entries.map((entry) => <tr key={entry.id}>
        ...
        <td style={td}>{entry.reference || "—"}</td><td style={td}>{money(entry.vat_amount)}</td><td style={{ ...td, textAlign:"right", fontWeight:800, color:entry.entry_type === "income" ? "#087A55" : "#B4233C" }}>{entry.entry_type === "income" ? "+" : "−"}{money(entry.amount)}</td>
...
function typeBadge(type:EntryType):CSSProperties { return {...badge,background:type === "income" ? "#EAF8F2" : type === "payout" ? "#FFF4E5" : "#FFF0F2",color:type === "income" ? "#087A55" : type === "payout" ? "#B54708" : "#B4233C"}; }

const page:CSSProperties={minHeight:"calc(100vh - 116px)",background:"linear-gradient(135deg,#EEF5FB 0%,#F8FAFC 45%,#EDF5F2 100%)",padding:"18px",fontFamily:"Arial,sans-serif",color:"#15263A"};
...
const buttonBase:CSSProperties={height:35,borderRadius:7,padding:"0 12px",fontSize:11,fontWeight:850,cursor:"pointer"}; const secondaryButton:CSSProperties={...buttonBase,border:"1px solid #B8C7D5",background:"#F5F8FA",color:"#28465F"}; const clearButton:CSSProperties={...buttonBase,border:"1px solid #E7B8BE",background:"#FFF5F6",color:"#A3293D"}; const processButton:CSSProperties={...buttonBase,border:0,background:"linear-gradient(135deg,#126A42,#198754)",color:"white",padding:"0 17px"};
...
const td:CSSProperties={padding:"9px 10px",borderBottom:"1px solid #EDF1F4",verticalAlign:"middle"}; const batchTd:CSSProperties={padding:3,borderBottom:"1px solid #DDE5EC",borderRight:"1px solid #EDF1F4"}; const rowNumberCell:CSSProperties={...batchTd,textAlign:"center",background:"#F4F7F9",fontWeight:800,color:"#647789"}; const gridInput:CSSProperties={width:"100%",height:33,border:"1px solid transparent",borderRadius:4,padding:"0 7px",fontSize:11,color:"#1D3347",background:"#FFF"}; const moneyInput:CSSProperties={...gridInput,textAlign:"right",minWidth:88}; const removeButton:CSSProperties={width:26,height:26,border:0,borderRadius:5,background:"#FFF0F2",color:"#B4233C",fontSize:17,cursor:"pointer"};
...
const vatGrid:CSSProperties={display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12,padding:16}; const note:CSSProperties={margin:"0 16px 16px",padding:11,borderRadius:8,background:"#FFF8E7",color:"#7A5B13",fontSize:11}; const errorBox:CSSProperties={padding:"9px 12px",marginBottom:10,borderRadius:8,background:"#FFF0F2",color:"#B4233C",fontSize:12,fontWeight:700}; const successBox:CSSProperties={padding:"9px 12px",marginBottom:10,borderRadius:8,background:"#EAF8F2",color:"#087A55",fontSize:12,fontWeight:700};
