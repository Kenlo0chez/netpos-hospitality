...
type CashbookRow = { id: string; date: string; description: string; reference: string; method: string; moneyIn: number; moneyOut: number; kind: "receipt" | "expense" };
...
export default function CashbookPage() {
  ...
  const loadCashbook = useCallback(async (selectedPropertyId: string) => {
    ...
    if (expenseResult.error) { setSetupRequired(true); setErrorMessage(expenseResult.error.message); }
    else if (paymentResult.error) setErrorMessage(paymentResult.error.message);
    else { setPayments((paymentResult.data as Payment[]) ?? []); setExpenses((expenseResult.data as Expense[]) ?? []); }
    setLoading(false);
  }, []);
  ...
  const initialise = useCallback(async () => {
    ...
    const selected = rows.some((p) => p.id === assigned) ? assigned! : rows[0]?.id ?? "";
    setProperties(rows); setPropertyId(selected);
  ...
  const rows = useMemo<CashbookRow[]>(() => {
    ...
    return [...receipts, ...costs].sort((a, b) => b.date.localeCompare(a.date));
  }, [payments, expenses]);
