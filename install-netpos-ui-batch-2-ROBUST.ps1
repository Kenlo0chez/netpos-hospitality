...
Write-Host ""
Write-Host "NETPOS HOSPITALITY - FINAL UI BATCH 2 (ROBUST FIX)" -ForegroundColor Cyan
...
$t = $t.Replace('maxWidth: 1280,', 'maxWidth: 1450,')
$t = $t.Replace('padding: "14px 20px 18px",', 'padding: "14px 22px 18px",')
$t = $t.Replace('color: "#111",', 'color: "#0D5FA8",')
$t = $t.Replace('color: "#6c7a89",', 'color: "#16885A",')
$t = $t.Replace('borderColor: "#ccc",', 'borderColor: "#C7D6E3",')
$t = $t.Replace('borderColor: "#ddd",', 'borderColor: "#D5E2ED",')
$t = $t.Replace('background: "#111",', 'background: "#0D5FA8",')
$t = $t.Replace('background: "#176332",', 'background: "#16885A",')

# Add page background only once.
if ($t -notmatch 'background:\s*"#F4F8FC"') {
    $old = '  boxSizing: "border-box",' + [Environment]::NewLine + '};'
    $new = '  boxSizing: "border-box",' + [Environment]::NewLine +
           '  background: "#F4F8FC",' + [Environment]::NewLine +
           '  color: "#17324D",' + [Environment]::NewLine +
           '  minHeight: "100vh",' + [Environment]::NewLine + '};'
    $t = $t.Replace($old, $new)
}

# Make title crystal-blue.
$oldTitle = 'const titleStyle: React.CSSProperties = {' + [Environment]::NewLine +
            '  margin: 0,' + [Environment]::NewLine +
            '  fontSize: 25,'
$newTitle = 'const titleStyle: React.CSSProperties = {' + [Environment]::NewLine +
            '  margin: 0,' + [Environment]::NewLine +
            '  fontSize: 25,' + [Environment]::NewLine +
            '  color: "#0D3F7A",'
...
$t = Read-Text $path
$t = $t.Replace('const BLUE = "#1557A6";', 'const BLUE = "#0D5FA8";')
$t = $t.Replace('const DARK_BLUE = "#0D3F7A";', 'const DARK_BLUE = "#0B477F";')
$t = $t.Replace('const GREEN = "#178A57";', 'const GREEN = "#16885A";')
$t = $t.Replace('const PAGE_BG = "#F4F7FB";', 'const PAGE_BG = "#F4F8FC";')
$t = $t.Replace('linear-gradient(135deg, #0D3F7A 0%, #1557A6 100%)', 'linear-gradient(135deg, #0B4E8A 0%, #0D668F 100%)')
Write-Text $path $t
...
$t = Read-Text $path
$t = $t.Replace('"#1557A6"', '"#0D5FA8"')
$t = $t.Replace('"#0D3F7A"', '"#0B477F"')
$t = $t.Replace('"#178A57"', '"#16885A"')
$t = $t.Replace('"#F4F7FB"', '"#F4F8FC"')
$t = $t.Replace('linear-gradient(135deg, #0D3F7A 0%, #1557A6 100%)', 'linear-gradient(135deg, #0B4E8A 0%, #0D668F 100%)')
Write-Text $path $t
...
Write-Host "Housekeeping, Reports, Users, X Report/EOD updated." -ForegroundColor Green
Write-Host "Setup created at app\setup\page.tsx." -ForegroundColor Green
Write-Host ""
